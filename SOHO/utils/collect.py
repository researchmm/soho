...
from .gather import gather_tensors_batch,collect_results_cpu
...
def nondist_forward_collect(func, data_loader, length):
    ...
    '''
    results = []
    prog_bar = commons.ProgressBar(len(data_loader))
...
def dist_forward_collect(func, data_loader, rank, length, ret_rank=-1):
    ...
    for k in results[0].keys():
        results_cat = np.concatenate([batch[k].numpy() for batch in results],
                                     axis=0)
        if ret_rank == -1:
            results_gathered=collect_results_cpu(results_cat,length)
            print(results_gathered)
            results_strip = np.concatenate(results_gathered, axis=0)
        else:
            results_gathered = gather_tensors_batch(
                results_cat, part_size=20, ret_rank=ret_rank)
            if rank == ret_rank:
