...
class AdamwSGD(Optimizer):
    """
    combine the SGD and Adamw into one optimizer to handle the CNN and transformer
    """
    def __init__(self,params,lr=1e-3,betas=(0.9,0.999),eps=1e-6,adamw_weight_decay=0.0,momentum=0.,dampening=0,sgd_weight_decay=0.,nesterov=False,correct_bias=True):
        if lr < 0.0:
        ...
        if sgd_weight_decay < 0.0:
            raise ValueError("Invalid weight_decay value: {}".format(sgd_weight_decay))

        defaults = dict(lr=lr, betas=betas, eps=eps, adamw_weight_decay=adamw_weight_decay, momentum=momentum,
                        dampening=dampening, sgd_weight_decay=sgd_weight_decay, nesterov=nesterov,
                        correct_bias=correct_bias)
        super(AdamwSGD, self).__init__(params, defaults)
    ...
    def step(self,closure=None):
        ...
        for group in self.param_groups:
            ...
            if opt in ['sgd',]:
                sgd_weight_decay = group['sgd_weight_decay']
                ...
                for p in group['params']:
                    ...
                    d_p = p.grad.data
                    if sgd_weight_decay !=0:
                        d_p = d_p.add(p.data, alpha=sgd_weight_decay)
                    if momentum !=0:
